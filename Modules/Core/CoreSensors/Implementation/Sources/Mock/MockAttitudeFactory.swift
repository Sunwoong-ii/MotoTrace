...
/// (heading, lean) → CMDeviceMotion(xTrueNorthZVertical, NWU 프레임)과 동일 규약의
...
/// 분석기가 시나리오의 lean 값을 기하학적으로 정확히 복원한다.
enum MockAttitudeFactory {
    ...
    static func attitude(headingDegrees: Double, leanDegrees: Double) -> Attitude {
        ...
        // lean: 전진축 기준 -lean 회전.
        // 부호는 LeanAngleAnalyzer의 atan2(dot(g0×g1, f), ...) 규약에서
        // 분석기 출력이 시나리오 lean과 부호까지 일치하도록 고정 — 단위 테스트로 핀 다운
        let half = -lean / 2
        let qLean = Quaternion(
            w: cos(half),
            x: sin(half) * forward.x,
            y: sin(half) * forward.y,
            z: sin(half) * forward.z
        )

        // 합성: yaw 먼저, lean 나중 (둘 다 월드 프레임 기준 회전)
        let q = qLean.multiplied(by: qYaw)
